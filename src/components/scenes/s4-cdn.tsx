...
function LargeFileProblemScene({ step }: { step: number }) {
  return (
    <div className="absolute inset-0">
      ...
      {step >= 1 && (
        <>
          <Packet from={USER} to={SERVER} label="GET exam-2023.pdf" variant="request" duration={1.5} />
          <motion.div
            initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.9 }}
            className="absolute flex items-center gap-2 font-mono text-[20px]"
            style={{ left: 900, top: 470, color: 'var(--color-red)' }}
          >
            <FileText size={40} /> 40 MB, streamed straight from the app server
          </motion.div>
        </>
      )}
    ...
    <div className="absolute inset-0">
      ...
      {step >= 2 && (
        <Packet
          from={SERVER}
          to={STORAGE}
          label="here's the file's address"
          variant="request"
          duration={1.5}
        />
      )}
...
function DistantOriginScene({ step }: { step: number }) {
  ...
  return (
    <div className="absolute inset-0">
      ...
      {step >= 1 && <Packet from={USER} to={ORIGIN} label="GET exam-2023.pdf" variant="request" duration={2} />}
      {step >= 2 && <Packet from={ORIGIN} to={USER} label="40 MB ·  slow" variant="data" duration={2.5} />}
    </div>
  );
}
...
function CdnEdgeScene({ step }: { step: number }) {
  ...
  return (
    <div className="absolute inset-0">
      <SceneTitle
        eyebrow="Part 5 · CDN"
        title="A CDN keeps copies close to the reader."
        sub="The first request fetches the file from the origin. After that, the nearby edge has a copy."
      />
      ...
      <Node
        ...
        type="cdn"
        label="Nearby Edge"
        sublabel={step >= 3 ? 'cached' : 'checking…'}
        status={step >= 3 ? 'healthy' : 'active'}
      />
      ...
      {/* Step 1: Student asks nearby CDN */}
      {step >= 1 && (
        <Packet
          from={USER}
          to={EDGE}
      ...
      {/* Step 2: Edge doesn't have it, asks Origin */}
      {step >= 2 && (
        <Packet
          from={EDGE}
          to={ORIGIN}
      ...
      {/* Step 3: Origin sends the file back to Edge */}
      {step >= 3 && (
        <Packet
          from={ORIGIN}
          to={EDGE}
      ...
      {/* Step 4: Edge serves it to Student */}
      {step >= 4 && (
        <Packet
          from={EDGE}
          to={USER}
      ...
      {step >= 4 && (
        ...
        >
          The next request never needs to go this far.
        </motion.p>
      )}
    </div>
  );
...
export const section4Scenes: SceneDef[] = [
  ...
  { id: 'object-storage', title: 'Object storage', steps: 4, Component: ObjectStorageScene },
  { id: 'distant-origin', title: 'A distant origin is still slow', steps: 3, Component: DistantOriginScene },
  { id: 'cdn-edge', title: 'CDN: cache near the reader', steps: 5, Component: CdnEdgeScene, notes: 'Core concept, emphasize distance, not magic.' },
];