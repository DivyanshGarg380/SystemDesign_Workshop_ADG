...
function AiProblemScene({ step }: { step: number }) {
  ...
  return (
    <div className="absolute inset-0">
      <SceneTitle
        eyebrow="Part 7 · AI-based Requests"
        title='LabXam adds "Explain this answer with AI"'
        sub="Each explanation is expensive to generate: real compute, real cost."
      />
      ...
      {/* User turns red once the spam starts building */}
      {step >= 1 && (
        <motion.div
          initial={{ opacity: 0 }}
      ...
      {/* AI turns red after the first request reaches it */}
      {step >= 1 && (
        <motion.div
          initial={{ opacity: 0 }}
      ...
      {step >= 1 && (
        <motion.p
          ...
          className="absolute font-mono text-[25px]"
          style={{
            left: 380,
            top: 620,
            color: 'var(--color-red)',
      ...
      <Prompt show={step >= 2}>
        What happens if one user sends 500 AI requests?
      </Prompt>
    </div>
  );
}
...
function RateLimitScene({ step }: { step: number }) {
  ...
  return (
    <div className="absolute inset-0">
      <SceneTitle
        eyebrow="Part 7 · AI-based Requests"
        title="A rate limit sits in front of expensive work"
        sub="Every student gets a fair share. No one can monopolise the AI service."
      />
      ...
      {/* Step 1: requests arrive at the rate limiter */}
      {step >= 1 &&
      ...
      {/* Step 2: gate decides only AFTER each request has arrived */}
      {step >= 2 &&
        Array.from({ length: 8 }).map((_, i) =>
          allowed.includes(i) ? (
            <Packet
      ...
      {step === 2 && (
        <motion.p
          ...
          className="absolute font-body text-[17px]"
          style={{
            left: 500,
            top: 700,
...
function AiQueueCacheScene({ step }: { step: number }) {
  ...
  return (
    <div className="absolute inset-0">
      <SceneTitle
        eyebrow="Part 7 · AI-based Requests"
        title="Allowed requests still shouldn't block the user"
        sub="Queue it, process it, and cache the answer for the next student who asks the same thing."
      />

      {/* Base states */}
      <Node x={USER.x} y={USER.y} type="user" label="Student" status="idle" />
      <Node x={QUEUE.x} y={QUEUE.y} type="queue" label="AI Job Queue" status="idle" />
      <Node x={WORKER.x} y={WORKER.y} type="worker" label="AI Worker" status="idle" />
      <Node x={CACHE.x} y={CACHE.y} type="cache" label="Cache" sublabel="answer, by question" status="idle" />
      ...
      {/* Step 1: Student sends request to queue */}
      {step >= 1 && (
        <Packet
          from={USER}
          to={QUEUE}
      ...
      {/* Queue becomes active when request arrives */}
      {step >= 1 && (
        <motion.div
          initial={{ opacity: 0 }}
      ...
      {/* Response only AFTER request reaches queue */}
      {step >= 1 && (
        <Packet
          from={QUEUE}
          to={USER}
          label="202, we'll notify you"
          variant="response"
          duration={1.7}
          delay={1.6}
        />
      ...
      {/* Step 2: Queue sends job to worker */}
      {step >= 2 && (
        <Packet
          from={QUEUE}
          to={WORKER}
      ...
      {/* Worker activates when job arrives */}
      {step >= 2 && (
        <motion.div
          initial={{ opacity: 0 }}
      ...
      {/* Step 3: Worker stores result in cache */}
      {step >= 3 && (
        <Packet
          from={WORKER}
          to={CACHE}
      ...
      {/* Cache becomes healthy when answer arrives */}
      {step >= 3 && (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1, transition: { delay: 1.7 } }}
        >
          <Node
            ...
            type="cache"
            label="Cache"
            sublabel="answer, by question"
            status="healthy"
          />
        </motion.div>
      )}

      {/* Step 4: Cached answer goes back to student */}
      {step >= 4 && (
        <Packet
          from={CACHE}
          to={USER}
      ...
      {step === 4 && (
        <motion.p
          initial={{ opacity: 0 }}
          animate={{ opacity: 1, transition: { delay: 1 } }}
          className="absolute font-display font-semibold text-[25px]"
          style={{
            left: 700,
            top: 700,
